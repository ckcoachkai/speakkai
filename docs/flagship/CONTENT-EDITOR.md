...
# Chinese resource maintenance (V24)

The existing English practice collection controls publication and timing for both languages. Chinese narrative copy lives in `src/data/storyChinese.ts` and `src/data/practiceChinese.ts`; it is source-managed, not automatically translated by the CMS. After editing English instructions, duration mentions or listener guidance, review the corresponding Chinese text before publication. A shared step count and inherited numeric minutes do not establish semantic parity.

Register completed translations explicitly in `src/lib/practiceAvailability.mjs`. Copy/registry key mismatch fails the build. Unpublishing a source removes both practice pages, index links, alternates and sitemap entries. Run `check-practice-content.mjs`, `check-site-languages.mjs` and the default flagship audits after rebuilding.
