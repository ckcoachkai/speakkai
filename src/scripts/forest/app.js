import {prepareSixSeven,sixSevenPose,SIX_SEVEN_DURATION,SIX_SEVEN_STYLES} from './six-seven.js';
import {composeTrumpFace,TRUMP_FACE_CROPS} from './trump-faces.js';
import {dancePose,DANCE_DURATION} from './dance.js';
...
import './style.css';
import {COLORS,MOODS,dress,drawTintedStudent,prepareDog,dogPose,drawBanana} from './visuals.js';
...
let runAge=0,running=false,ready=false,time=0,last=performance.now(),musicOn=true,effectsOn=true,selectionTimer,biteUntil=0;
const soundEffects=createForestEffects(),ketchup=createKetchupParticles(),magic=createWolfMagic();
let sixSevenArt=[],sixSevenRound=-1;
const trumpFaces=[];let trumpFaceIndex=-1;
function nextTrumpFace(){if(hostId!=='trump'||!trumpFaces.length)return;trumpFaceIndex=(trumpFaceIndex+1)%trumpFaces.length;art['host-trump']=trumpFaces[trumpFaceIndex];bossMeshes.trump=prepareBossRig(art['host-trump'],'trump');}
...
// Original 126 BPM major-key pop/EDM fallback; Suno track integration pending access.
let quoteStart=0;
let audio,master,musicGain,musicMeter,beatIndex=0,nextBeat=0,audioStarted=false;
const BEAT=60/126/2,scale=[72,76,79,76,74,79,83,79,69,72,76,72,65,69,72,77];
...
function schedule(){if(!audio||audio.state!=='running'||paused)return;if(nextBeat<audio.currentTime-.4)nextBeat=audio.currentTime+.03;while(nextBeat<audio.currentTime+.15){const b=beatIndex,t=nextBeat;if(musicOn){const midi=scale[b%scale.length];tone(440*2**((midi-69)/12),t,.48,'sine',.17);tone(440*2**((midi+12-69)/12),t+.012,.19,'triangle',.055);if(b%2===0){tone([130.81,98,110,87.31][Math.floor(b/8)%4],t,.22,'sawtooth',.09);const o=audio.createOscillator(),g=audio.createGain();o.frequency.setValueAtTime(130,t);o.frequency.exponentialRampToValueAtTime(40,t+.16);g.gain.setValueAtTime(.26,t);g.gain.exponentialRampToValueAtTime(.001,t+.22);o.connect(g);g.connect(musicGain);o.start(t);o.stop(t+.23);}else{tone(180,t,.07,'triangle',.10);tone(2200,t+.31,.045,'square',.012);}if(b%8===0){const root=[60,55,57,53][Math.floor(b/8)%4];for(const interval of [0,4,7])tone(440*2**((root+interval-69)/12),t,BEAT*6,'triangle',.05);}}beatIndex++;nextBeat+=BEAT;}}
async function unlock(){try{if(!audio){audio=new (window.AudioContext||window['webkitAudioContext'])();master=audio.createGain();musicGain=audio.createGain();musicGain.gain.value=musicOn?.65:0;musicMeter=audio.createAnalyser();musicMeter.fftSize=256;musicGain.connect(musicMeter);musicMeter.connect(master);const limiter=audio.createDynamicsCompressor();limiter.threshold.value=-12;limiter.ratio.value=8;master.connect(limiter);limiter.connect(audio.destination);master.gain.value=Number($('#volume').value)/100;}await audio.resume();if(!audioStarted){audioStarted=true;nextBeat=audio.currentTime+.03;setInterval(schedule,50);schedule();}await soundEffects.load(audio,master).catch(()=>{$('#effects').title='Effects could not load. Toggle off and on to retry.';});}catch(e){$('#status').textContent='Sound is unavailable in this browser; the picker still works.';console.error(e);}}
$('#music').onclick=()=>{if(!audioStarted){musicOn=true;unlock();return;}musicOn=!musicOn;$('#music').textContent=musicOn?'♫ Music on':'♫ Music off';$('#music').setAttribute('aria-pressed',musicOn);if(musicGain)musicGain.gain.setTargetAtTime(musicOn?.65:0,audio.currentTime,.04);if(musicOn)unlock();};$('#volume').oninput=()=>{if(master)master.gain.setTargetAtTime(Number($('#volume').value)/100,audio.currentTime,.02);};
...
const hostSelect=$('#host-select');HOSTS.forEach(h=>hostSelect.add(new Option(h.name,h.id)));hostSelect.value=hostId;hostSelect.onchange=()=>{hostId=hostSelect.value;magic.reset(time);try{localStorage.setItem('forest-host',hostId);}catch{}DOG_PHRASES=currentHost().phrases;quoteStart=time;};
...
function frame(now){
 ...
 const current=Math.floor(Math.max(0,time-quoteStart)/7)%DOG_PHRASES.length;
 const open=paused?mouthOpen:(mouthOpen=time<biteUntil&&Math.sin((biteUntil-time)*30)>0);
 $('#dog-speech').textContent=DOG_PHRASES[current];
 ...
 const encounterAge=encounter?time-encounter.start:-1,hit=punchAt(encounterAge);const dancing=encounter&&!encounter.shown&&encounterAge>=ENCOUNTER_DURATION;const finaleAge=encounterAge-ENCOUNTER_DURATION-DANCE_DURATION;const finale=dancing&&finaleAge>=0?sixSevenPose(finaleAge,gentle):null;const dance=dancing&&!finale?dancePose(encounterAge-ENCOUNTER_DURATION,gentle):null;
 ...
 if(dance){Object.assign(pose,{jump:dance.jump,sway:dance.sway,angles:dance.angles,dance,recoil:0});$('#status').textContent='Dance break · '+(dance.phase==='shoulders'?'Shake your shoulders!':dance.phase==='hips'?'Move your hips!':'Jump to the beat!');}
 if(finale){Object.assign(pose,{jump:0,sway:0,angles:hostId==='dog'?finale.angles.map(a=>-a):finale.angles,recoil:0,sixSeven:{sprites:sixSevenArt[Math.max(0,sixSevenRound)],pop:finale.pop}});$('#status').textContent='6–7!';}
 ...
 for(const s of sorted){
  ...
  if(running&&!paused){
   const contact=advanceRunner(s,dt*Number($('#run-speed').value),runAge);
  ...
  drawTintedStudent(ctx,body,faces[artNames[model]][expression],r.w,r.h,s.phase,s.distance>0,s.boost,gentle,0,{cape:s.cape,leap:r.leap,clock:time,seed:s.i,punch:attack,charging:s.distance>0&&(running||!!encounter)});
 ...
 if(encounter&&!encounter.shown&&time-encounter.start>=ENCOUNTER_DURATION+DANCE_DURATION+SIX_SEVEN_DURATION)revealResult();
...
window.forestApp={getState:()=>({ready,running,paused,hostId,trumpFaceIndex,sixSevenStyle:sixSevenRound,sixSevenActive:!!encounter&&!encounter.shown&&time-encounter.start>=ENCOUNTER_DURATION+DANCE_DURATION,musicLevel:musicGain?.gain.value||0,musicBeat:beatIndex,musicRms:(()=>{if(!musicMeter)return 0;const a=new Float32Array(256);musicMeter.getFloatTimeDomainData(a);return Math.sqrt(a.reduce((n,v)=>n+v*v,0)/a.length);})(),dancePhase:encounter&&!encounter.shown&&time-encounter.start>=ENCOUNTER_DURATION?dancePose(time-encounter.start-ENCOUNTER_DURATION,gentle).phase:null,attackZone:encounter?getAttackTarget(currentHost(),Math.max(0,CRUNCH_BEATS.findLastIndex(b=>time-encounter.start>=b-.10))).zone:null,reaction:REACTIONS[reactionIndex],runSpeed:Number($('#run-speed').value),skeletonHits:students.reduce((n,s)=>n+(s.skeleton?.hits||0),0),headHinge:{...headHinge},capeModel:'3d-constraint-cloth',bossAreaMultiplier:WOLF_SCALE**2,bossRig:hostId==='dog'?'layered':'skinned',sceneTime:time,encounter:encounter?{...encounter,age:time-encounter.start,punch:punchAt(time-encounter.start)}:null,magic:{...magic.stats},wolfAreaMultiplier:WOLF_SCALE**2,cameraScale:WORLD.cameraScale,worldWidth:W,effectsOn,sfx:{...soundEffects.stats},ketchupParticles:ketchup.count,names:[...names],chosen:[...chosen],activeAction,looks,bananasVisible:bananas.filter(b=>!b.eaten&&students[b.i]?.visible).length,musicOn,voiceOn:false,quote:DOG_PHRASES[Math.floor(Math.max(0,time-quoteStart)/7)%DOG_PHRASES.length],audioState:audio?.state,students:students.map(({i,progress,boost,currentExpression,distance,speed,phase,renderX,renderY,capeState,visible})=>({i,progress,boost,currentExpression,distance,speed,phase,renderX,renderY,capeState,visible}))})};
