...
function frame(now){
 ...
 const encounterAge=encounter?time-encounter.start:-1,hit=punchAt(encounterAge);const dancing=encounter&&!encounter.shown&&encounterAge>=ENCOUNTER_DURATION;const finaleAge=encounterAge-ENCOUNTER_DURATION;const finale=dancing&&finaleAge>=0?sixSevenPose(finaleAge,gentle):null;const dance=dancing?dancePose(encounterAge-ENCOUNTER_DURATION,gentle):null;
 ...
 if(finale){Object.assign(pose,{angles:hostId==='dog'?finale.angles.map(a=>-a):finale.angles,recoil:0,sixSeven:{sprites:sixSevenArt[Math.max(0,sixSevenRound)],pop:finale.pop}});$('#status').textContent='6–7!';}
 ...
 if(encounter&&!encounter.shown&&time-encounter.start>=ENCOUNTER_DURATION+SIX_SEVEN_DURATION)revealResult();
