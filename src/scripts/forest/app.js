...
function start(){if(!ready||running)return;nextTrumpFace();names=names.map((n,i)=>n.trim()||'Student '+(i+1));storeNames();unlock();if(chosen.length===names.length)chosen.length=0;setup();runAge=0;paused=false;running=true;$('#pause').disabled=false;$('#pause').textContent='Pause';$('#result').hidden=true;$('#start').disabled=true;$('#start').textContent='Running…';$('#status').textContent=`${names.length-chosen.length} students running toward ${currentHost().name}`;roster();history();}
...
function frame(now){
 ...
 const encounterAge=encounter?time-encounter.start:-1,hit=punchAt(encounterAge);const dancing=encounter&&!encounter.shown&&encounterAge>=ENCOUNTER_DURATION;const dance=dancing?dancePose(encounterAge-ENCOUNTER_DURATION,gentle):null;
 ...
 if(encounter&&!encounter.shown&&time-encounter.start>=ENCOUNTER_DURATION+DANCE_DURATION)revealResult();
