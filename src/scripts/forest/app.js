...
let audio,master,musicGain,voiceGain,musicMeter,samples={},beatIndex=0,nextBeat=0,audioStarted=false;
...
async function unlock(){try{if(!audio){audio=new (window.AudioContext||window['webkitAudioContext'])();master=audio.createGain();musicGain=audio.createGain();voiceGain=audio.createGain();musicGain.gain.value=musicOn?.65:0;voiceGain.gain.value=voiceOn?1:0;musicMeter=audio.createAnalyser();musicMeter.fftSize=256;musicGain.connect(musicMeter);musicMeter.connect(master);voiceGain.connect(master);const limiter=audio.createDynamicsCompressor();limiter.threshold.value=-12;limiter.ratio.value=8;master.connect(limiter);limiter.connect(audio.destination);audio.resume();master.gain.value=Number($('#volume').value)/100;}await audio.resume();if(!audioStarted){audioStarted=true;nextBeat=audio.currentTime+.03;nextPhraseAt=audio.currentTime+.12;setInterval(schedule,50);schedule();}await Promise.all([loadVoice(hostId),soundEffects.load(audio,master).then(()=>{$('#effects').title='Footsteps, cartoon crunch punches and banana boosts ready';}).catch(()=>{$('#effects').title='Effects could not load. Toggle off and on to retry.';})]);if(!audioStarted){audioStarted=true;nextBeat=audio.currentTime+.12;nextPhraseAt=audio.currentTime+.12;setInterval(schedule,50);}}catch(e){$('#status').textContent='Sound is unavailable in this browser; the picker still works.';console.error(e);}}
