...
async function start(){if(!ready||running||starting)return;starting=true;$('#start').disabled=true;await unlock();starting=false;sixSevenRound=(sixSevenRound+1)%SIX_SEVEN_STYLES;nextTrumpFace();names=names.map((n,i)=>n.trim()||'Student '+(i+1));storeNames();unlock();if(chosen.length===names.length)chosen.length=0;setup();runAge=-1.2;paused=false;running=true;$('#pause').disabled=false;$('#pause').textContent='Pause';$('#result').hidden=true;$('#start').disabled=true;$('#start').textContent='Running…';$('#status').textContent=`${names.length-chosen.length} students running toward ${currentHost().name}`;roster();history();}
...
// The first interaction enables lobby music under browser autoplay rules.
document.addEventListener('pointerdown',()=>{if(!audioStarted)unlock();},{once:true});
