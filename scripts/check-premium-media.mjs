...
const illustrations=JSON.parse(await readFile('media/visual-assets.json','utf8'));
for(const record of illustrations){const bytes=await readFile(`dist/media/${record.file}`);assert.equal(bytes.length,record.bytes);assert.equal(createHash('sha256').update(bytes).digest('hex'),record.sha256);assert.equal(record.fictional,true);}
