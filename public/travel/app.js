...
function events(rows){return rows.map(([t,title,note])=>`<div class="event"><time>${t}</time><div><h4>${tr(title)}</h4><p>${tr(note)}</p></div></div>`).join('');}
...
document.querySelector('#costs').innerHTML=costs.map(c=>`<label class="cost-row" for="${c.key}"><span>${tr(c.name)} (CNY)</span><input type="number" min="0" max="100000" step="10" id="${c.key}" value="${c.value}"></label>`).join('');
function budget(){const spa=document.querySelector('#spa').checked;const values=costs.map(c=>Math.min(100000,Math.max(0,Number(document.getElementById(c.key).value)||0)));if(!spa)values[3]=0;document.querySelector('#spaCost').disabled=!spa;const total=values.reduce((a,b)=>a+b,0);document.querySelector('#total').textContent=yuan(total);document.querySelector('#remaining').textContent=total<=1500?yuan(1500-total)+tr(' left within your target'):yuan(total-1500)+tr(' over your target');document.querySelector('#remaining').style.color=total>1500?'#ffd0b8':'#d7ebd8';document.querySelector('#bars').innerHTML=costs.map((c,i)=>`<div class="bar-row">${tr(c.name)} · ${yuan(values[i])}<div class="bar-track"><div class="bar-fill" style="width:${total?values[i]/total*100:0}%"></div></div></div>`).join('');}
document.querySelector('#costs').addEventListener('input',budget);document.querySelector('#spa').addEventListener('change',()=>{renderPlan();budget();});let printDetails=[];window.addEventListener('beforeprint',()=>{printDetails=[...document.querySelectorAll('details')].filter(d=>!d.open);printDetails.forEach(d=>d.open=true);});window.addEventListener('afterprint',()=>printDetails.forEach(d=>d.open=false));document.querySelector('#print').onclick=()=>window.print();
document.querySelector('#calendar').onclick=()=>{const rows=[...day1.map(e=>({day:'20260916',event:e})),...day2.map(e=>({day:'20260917',event:e}))];const lines=['BEGIN:VCALENDAR','VERSION:2.0','PRODID:-//SpeakKai//Travel Guide//EN','CALSCALE:GREGORIAN'];for(const [i,{day,event}] of rows.entries()){if(i===5&&!document.querySelector('#spa').checked)continue;const [time,title,note]=event;const stamp=day+'T'+time.replace(':','')+'00';const escape=s=>s.replaceAll('\\','\\\\').replaceAll(';','\\;').replaceAll(',','\\,');lines.push('BEGIN:VEVENT',`UID:suzhou-20260916-${i}@speakkai.com`,'DTSTAMP:20260915T080000Z',`DTSTART;TZID=Asia/Shanghai:${stamp}`,`SUMMARY:${escape(tr(title))}`,`DESCRIPTION:${escape(tr('Proposed itinerary. ')+tr(note))}`,'STATUS:TENTATIVE','END:VEVENT');}lines.push('END:VCALENDAR');const url=URL.createObjectURL(new Blob([lines.join('\r\n')+'\r\n'],{type:'text/calendar;charset=utf-8'}));const a=document.createElement('a');a.href=url;a.download='suzhou-bay-september-16-17.ics';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);};renderPlan();budget();

document.addEventListener("travel-language",()=>{renderPlan();budget();costs.forEach(c=>document.querySelector(`label[for="${c.key}"] span`).textContent=tr(c.name)+" (CNY)");});
