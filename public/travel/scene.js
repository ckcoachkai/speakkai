...
try{
 ...
 function loadModel(){requested=true;new GLTFLoader().load('/travel/assets/suzhou-route.glb',g=>{scene.add(g.scene);loaded=true;wrap.querySelector('.map-poster').hidden=true;note.textContent='Artist’s route sketch · Drag to explore · Pinch to zoom';},undefined,()=>{canvas.hidden=true;note.textContent='Explore the illustrated route or open the live map for directions.';document.querySelectorAll('[data-view],#reset').forEach(b=>b.disabled=true);});}
